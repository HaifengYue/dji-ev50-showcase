...
export function environment(scene:T.Scene){
 ...
 const height=(x:number,z:number)=>Math.min(1,Math.max(0,(Math.hypot(x,z)-10)/35))*(1.6+1.2*Math.sin(x*.035)*Math.cos(z*.027)+.35*Math.sin(x*.13+z*.09));
 ...
 for(let j=0;j<=M;j++)for(let i=0;i<=N;i++){const a=i*2*Math.PI/N,r=235+k*170+j*3.8,h=(42+46*Math.sin(a*3+k*1.3)**2+26*Math.sin(a*7+k)**2+10*Math.sin(a*17+r*.038)+5*Math.sin(a*41+r*.18))*Math.max(0,Math.sin(Math.PI*j/M))**1.5;positions.push(r*Math.cos(a),h-2,r*Math.sin(a));
 const c=new T.Color(k===0?0x58634c:k===1?0x748080:0x94a4ad);c.lerp(new T.Color(0xaaa99d),T.MathUtils.smoothstep(h,45,85)*.7);c.lerp(new T.Color(0xe6e8e2),T.MathUtils.smoothstep(h+8*Math.sin(a*27),92,117));c.multiplyScalar(.88+.12*Math.sin(r*.13+a*22));cols.push(c.r,c.g,c.b);}
 ...
 const trunks=new T.InstancedMesh(new T.CylinderGeometry(.11,.18,4,5),mat(0x655749),480),crowns=new T.InstancedMesh(foliage,mat(0x344b3a),480);
 for(let i=0;i<480;i++){let x:number,z:number;do{x=-200+rand()*420;z=-220+rand()*440;}while(Math.hypot(x,z)<28||Math.abs(x-streamX(z))<9);const s=.55+rand()*.6,y=height(x,z);dummy.rotation.set(0,rand()*6.28,0);dummy.scale.setScalar(s);dummy.position.set(x,y+2*s,z);dummy.updateMatrix();trunks.setMatrixAt(i,dummy.matrix);dummy.position.y=y+2.4*s;dummy.updateMatrix();crowns.setMatrixAt(i,dummy.matrix);crowns.setColorAt(i,new T.Color().setHSL(.28+rand()*.05,.18+rand()*.18,.18+rand()*.10));}
 trunks.castShadow=crowns.castShadow=true;trunks.receiveShadow=crowns.receiveShadow=true;group.add(trunks,crowns);
 const sky=new Sky();sky.scale.setScalar(1500);const u=sky.material.uniforms;u.turbidity.value=3.2;u.rayleigh.value=1.4;u.mieCoefficient.value=.004;u.mieDirectionalG.value=.82;u.sunPosition.value.set(-.42,.64,.34);group.add(sky);
 return{group,setQuality(q:string){grass.count=q==='Low'?500:q==='Medium'?2600:6500;rocks.count=q==='Low'?150:q==='Medium'?450:900;trunks.count=crowns.count=q==='Low'?120:q==='Medium'?280:480;mountains.children[2].visible=q!=='Low';detail.anisotropy=q==='High'?8:2;}};
}
