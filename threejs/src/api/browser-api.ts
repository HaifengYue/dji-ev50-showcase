...
export function installBrowserApi<State>(gateway:Ev50ApiGateway<State>,subscribe:(callback:StateSubscriber<State>)=>()=>void):LegacyApi<State>{
 const request=(request:ApiRequest)=>gateway.request(request);
 const state=(operation:string,payload?:unknown)=>unwrap(request({operation,payload}) as ApiResponse<State>);
 const api:LegacyApi<State>={
  get ready(){const response=request({operation:'system.health'}) as ApiResponse<{ready:boolean}>;return response.ok&&response.data.ready;},
  request,
  ...
  euler:(roll,pitch,yaw)=>{
   ...
   const cy=Math.cos(yaw*.5),sy=Math.sin(yaw*.5),cp=Math.cos(pitch*.5),sp=Math.sin(pitch*.5),cr=Math.cos(roll*.5),sr=Math.sin(roll*.5);
   // Three.js Euler YXZ: pitch=X, yaw=Y, roll=Z.
   return state('flight.command',{type:'attitude',quaternion:[sp*cy*cr+cp*sy*sr,cp*sy*cr-sp*cy*sr,cp*cy*sr-sp*sy*cr,cp*cy*cr+sp*sy*sr]});
  },
  setRoute:(route)=>state('mission.select',{route}),
  play:()=>state('flight.play'),pause:()=>state('flight.pause'),resume:()=>state('flight.resume'),reset:()=>state('flight.reset'),
  seek:(seconds)=>state('flight.seek',{seconds}),setSpeed:(speed)=>state('flight.speed',{speed}),subscribe
 };
 window.addEventListener('ev50-command',event=>{
  const detail=(event as CustomEvent).detail as ApiRequest|{id?:string;command?:FlightControlCommand}|FlightControlCommand|undefined;
  const isStructured=!!detail&&typeof detail==='object'&&'operation' in detail;
  const request=isStructured
   ? detail as ApiRequest
   : {id:(detail as {id?:string})?.id,operation:'flight.command',payload:(detail as {command?:FlightControlCommand})?.command??detail};
