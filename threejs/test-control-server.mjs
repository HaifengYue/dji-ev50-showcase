...
const port = 18987;
const child = spawn(process.execPath, ['control-server.mjs', String(port)], {
...
try {
  ...
  const queued = await json('/api/v1/flight/commands', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ id: 'move-1', type: 'position', position: [10, 20, 30] }),
  });
  assert.equal(queued.status, 202);
  assert.equal(queued.body.data.status, 'queued');
  const commands = await json('/api/v1/bridge/commands?after=0');
  assert.deepEqual(commands.body.data.commands[0].payload, {
    type: 'position',
    position: [10, 20, 30],
  });
  await json('/api/v1/bridge/state', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
  ...
  await json('/api/v1/bridge/results', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
      id: 'move-1',
      sequence: 1,
      response: { ok: true, data: { position: [10, 20, 30] } },
    }),
  });
