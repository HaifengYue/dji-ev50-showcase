...
本项目包含 Blender 建模工程、GLB 模型，以及 Vite + TypeScript + Three.js 交互式展示页面。当前为 **v09 / API 3.3**，浏览器预设任务为 **180 秒**。这是非官方视觉演示，模型细节为参考图像基础上的独立设计，不是工程 CAD 或真实飞行控制软件。
...
录屏只包含三维画面，无 HTML 面板或声音，最长 200 秒；输出格式由浏览器支持决定。按 Escape 可退出沉浸展示，录制期间沉浸视图保留停止按钮。API 3.3 新增 MAVLink 风格 JSONL 日志导入/导出、统一实时遥测通道、记录回放、机体/相机描述与场景查询；它们仅驱动浏览器内的可视化，不向真实飞行器发送指令。本机 MAVLink 风格控制服务与脚本见 [MAVLINK_LOCAL.md](docs/MAVLINK_LOCAL.md)。
